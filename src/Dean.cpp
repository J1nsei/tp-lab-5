...
void Dean::transfer(string from, string where) {
	if (from == where) {
		cout << "Enter different groups!" << endl;
		return;
	}
	...
	for (auto& group : groups) {
		if (group.getName() == from) {
			...
			std::cout << transferred_student.getName() << " was transfered from " << from << " to " << where;
			break;
		}
	}
	for (auto& group : groups) {
...
void Dean::statistics() {
	...
	cout << endl;
	for (auto& group : groups) {
		cout << "Average mark of " << group.getName() << " - " << group.averageMark() << endl;
	...
	cout << endl;
	cout << "Best group is " << best_group->getName() << "! With an average mark - " << best_group->averageMark() << endl << endl;
	cout << "Best students from all groups:" << endl;
	scholarship();
}
