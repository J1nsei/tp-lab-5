...
TEST(group_test, transfer_test) {
	...
	Student* st = new Student("1 Кузьмин Никита Сергеевич", NULL);
	gr1->add_student(st);
...
TEST(group_test, expell_test) {
	Group* gr = new Group("18PI");
	Student* st = new Student("1 Шабров Илья Дмитриевич", NULL);
	gr->add_student(st);
...
TEST(group_test, search_test) {
	Group* gr = new Group("18PI");
	Student* st1 = new Student("1 A1 A2 A3", NULL);
	Student* st2 = new Student("2 B1 B2 B3", NULL);
	Student* st3 = new Student("3 C1 C2 C3", NULL);
	gr->add_student(st1);
	gr->add_student(st2);
	gr->add_student(st3);
